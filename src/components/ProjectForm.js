...
async function checkRepo(repolink) {
  ...
  try {
    ...
    if (numOfIssues < 2)
      returnMsg[
        'message'
      ] = `Repo has only ${numOfIssues} issues, Please maintain atleast 2 issues. `;
  } catch (err) {
    returnMsg[
      'message'
    ] = `The link should be in format of https://github.com/USERNAME/REPO. Please add a valid Github link repo with atleast 2 open issues. `;
  }
  ...
  try {
    ...
    if (res.data.size < 100)
      returnMsg[
        'message'
      ] += `Please add a more descriptive modified README. It is too short`;
  } catch (err) {
    returnMsg['message'] += `Please add a descriptive README.md`;
  }

  if (returnMsg['message'] === '') return returnMsg;
