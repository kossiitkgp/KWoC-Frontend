...
function Navbar() {
  ...
  return (
    <div className="fixed inset-x-0 z-50 w-full">
      <nav className={`flex shadow-md mt-4 items-center justify-between max-w-7xl gap-4 mx-auto border border-[#FFFFFF]/[0.16] px-4 py-2 rounded-lg w-[95%] inset-x-0 backdrop-blur-md z-50 ${menuOpen ? 'hidden' : null}`}>
        ...
        <div className={`${menuOpen ? 'hidden' : null}`}>
          <ul className={`lg:flex lg:items-center lg:w-auto lg:space-x-6 ${menuOpen ? 'block' : 'hidden'}`}>
            ...
            <IconContext.Provider value={{ size: '3em' }}>
              <Link to=''>
                <CiLogin color='#dc2626' className={`transition-transform transform ${isHovered ? 'drop-shadow-glow scale-110' : 'scale-100'}`}
      ...
      {menuOpen && <div className='navbar-menu w-full z-50 h-screen transition-transform transform ease-in-out duration-500 translate-x-0 p-2 flex justify-end'>
